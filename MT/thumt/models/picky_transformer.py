...
class PickyAttentionSubLayer(modules.Module):
    ...
    def _prune_heads(self, heads):
        ...
        # Prune linear layers
        self.attention.q_transform = prune_linear_layer(self.attention.q_transform, index, scale=True)
        self.attention.k_transform = prune_linear_layer(self.attention.k_transform, index, scale=True)
        self.attention.v_transform = prune_linear_layer(self.attention.v_transform, index, scale=True)
        self.attention.o_transform = prune_linear_layer(self.attention.o_transform, index, dim=1, scale=True)

...
class FFNSubLayer(modules.Module):
    ...
    def __init__(self, params, dtype=None, name="ffn_layer"):
        ...
        ffn_filter_size = 4 * ffn_hidden_size

        with utils.scope(name):
            self.ffn_input_transform = modules.Affine(params.hidden_size, ffn_hidden_size)
            self.ffn_layer = modules.FeedForward(ffn_hidden_size,
                                                 ffn_filter_size,
                                                 dropout=params.relu_dropout)
            self.ffn_output_transform = modules.Affine(ffn_hidden_size, params.hidden_size)
            self.layer_norm = modules.LayerNorm(params.hidden_size)
    ...
    def forward(self, x):
        ...
        y = self.ffn_input_transform(y)
        y = self.ffn_layer(y)
        y = self.ffn_output_transform(y)
        y = nn.functional.dropout(y, self.dropout, self.training)
    ...
    def reset_parameters(self):
        nn.init.xavier_uniform_(self.ffn_input_transform.weight)
        nn.init.xavier_uniform_(self.ffn_output_transform.weight)
        nn.init.constant_(self.ffn_input_transform.bias, 0.0)
        nn.init.constant_(self.ffn_output_transform.bias, 0.0)


class ThinFFNTransformerEncoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(ThinFFNTransformerEncoderLayer, self).__init__(name=name)
...
class ThinFFNTransformerDecoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(ThinFFNTransformerDecoderLayer, self).__init__(name=name)
...
class ThinFFNTransformerEncoder(modules.Module):

    def __init__(self, params, name="encoder"):
        super(ThinFFNTransformerEncoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                ThinFFNTransformerEncoderLayer(params, name="layer_%d" % i)
...
class ThinFFNTransformerDecoder(modules.Module):

    def __init__(self, params, name="decoder"):
        super(ThinFFNTransformerDecoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                ThinFFNTransformerDecoderLayer(params, name="layer_%d" % i)
...
class ThinFFNTransformer(modules.Module):

    def __init__(self, params, name="thinffn_transformer"):
        super(ThinFFNTransformer, self).__init__(name=name)
        self.params = params

        with utils.scope(name):
            self.build_embedding(params)
            self.encoding = modules.PositionalEmbedding()
            self.encoder = ThinFFNTransformerEncoder(params)
            self.decoder = ThinFFNTransformerDecoder(params)
    ...
    @staticmethod
    def base_params_v2():
        params = ThinFFNTransformer.base_params()
    ...
    @staticmethod
    def big_params_v2():
        params = ThinFFNTransformer.base_params_v2()
        params.hidden_size = 1024
        params.filter_size = 4096
        params.num_heads = 16
        params.residual_dropout = 0.3
    ...
    def default_params(name=None):
        if name == "base":
            return ThinFFNTransformer.base_params()
        elif name == "base_v2":
            return ThinFFNTransformer.base_params_v2()
        elif name == "big":
            return ThinFFNTransformer.big_params()
        elif name == "big_v2":
            return ThinFFNTransformer.big_params_v2()
        elif name == "test":
            return ThinFFNTransformer.test_params()
        else:
            return ThinFFNTransformer.base_params()

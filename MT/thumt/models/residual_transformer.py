...
class FFNSubLayer(modules.Module):
    ...
    def __init__(self, params, dtype=None, name="ffn_layer"):
        ...
        self.dropout = params.residual_dropout
        self.normalization = params.normalization
        self.residual = params.ffn_residual
...
class ResidualTransformerEncoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(ResidualTransformerEncoderLayer, self).__init__(name=name)

        self.residual = params.skip_residual
...
class ResidualTransformerDecoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(ResidualTransformerDecoderLayer, self).__init__(name=name)
...
class ResidualTransformerEncoder(modules.Module):

    def __init__(self, params, name="encoder"):
        super(ResidualTransformerEncoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                ResidualTransformerEncoderLayer(params, name="layer_%d" % i)
...
class ResidualTransformerDecoder(modules.Module):

    def __init__(self, params, name="decoder"):
        super(ResidualTransformerDecoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                ResidualTransformerDecoderLayer(params, name="layer_%d" % i)
...
class ResidualTransformer(modules.Module):

    def __init__(self, params, name="residual_transformer"):
        super(ResidualTransformer, self).__init__(name=name)
        self.params = params

        with utils.scope(name):
            self.build_embedding(params)
            self.encoding = modules.PositionalEmbedding()
            self.encoder = ResidualTransformerEncoder(params)
            self.decoder = ResidualTransformerDecoder(params)
    ...
    @staticmethod
    def base_params_v2():
        params = ResidualTransformer.base_params()
    ...
    @staticmethod
    def big_params_v2():
        params = ResidualTransformer.base_params_v2()
        params.hidden_size = 1024
        params.filter_size = 4096
        params.num_heads = 16
        params.residual_dropout = 0.3
    ...
    def default_params(name=None):
        if name == "base":
            return ResidualTransformer.base_params()
        elif name == "base_v2":
            return ResidualTransformer.base_params_v2()
        elif name == "big":
            return ResidualTransformer.big_params()
        elif name == "big_v2":
            return ResidualTransformer.big_params_v2()
        elif name == "test":
            return ResidualTransformer.test_params()
        else:
            return ResidualTransformer.base_params()

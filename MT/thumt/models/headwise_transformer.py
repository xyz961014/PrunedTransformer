...
class HeadWiseAttentionSubLayer(modules.Module):

    def __init__(self, params, name="attention"):
        super(HeadWiseAttentionSubLayer, self).__init__(name=name)
        ...
        with utils.scope(name):
            self.attention = modules.HeadWiseMultiHeadAttention(params.hidden_size,
                                                                params.head_size,
                                                                params.num_heads, 
...
class HeadWiseFFNSubLayer(modules.Module):

    def __init__(self, params, dtype=None, name="ffn_layer"):
        super(HeadWiseFFNSubLayer, self).__init__(name=name)
        ...
        with utils.scope(name):
            ...
            if self.normalization == "before":
                ...
                self.layer_norm = modules.LayerNorm(params.hidden_size)
            self.residual_transform = modules.Affine(ffn_hidden_size, params.hidden_size,
                                                 name="residual_transform")
...
class HeadWiseTransformerEncoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(HeadWiseTransformerEncoderLayer, self).__init__(name=name)

        with utils.scope(name):
            self.self_attention = HeadWiseAttentionSubLayer(params)
            self.feed_forward = HeadWiseFFNSubLayer(params)
...
class HeadWiseTransformerDecoderLayer(modules.Module):

    def __init__(self, params, name="layer"):
        super(HeadWiseTransformerDecoderLayer, self).__init__(name=name)
        ...
        with utils.scope(name):
            ...
            self.encdec_attention = HeadWiseAttentionSubLayer(params,
...
class HeadWiseTransformerEncoder(modules.Module):

    def __init__(self, params, name="encoder"):
        super(HeadWiseTransformerEncoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                HeadWiseTransformerEncoderLayer(params, name="layer_%d" % i)
...
class HeadWiseTransformerDecoder(modules.Module):

    def __init__(self, params, name="decoder"):
        super(HeadWiseTransformerDecoder, self).__init__(name=name)

        self.normalization = params.normalization

        with utils.scope(name):
            self.layers = nn.ModuleList([
                HeadWiseTransformerDecoderLayer(params, name="layer_%d" % i)
...
class HeadWiseTransformer(modules.Module):

    def __init__(self, params, name="headwise_transformer"):
        super(HeadWiseTransformer, self).__init__(name=name)
        self.params = params

        with utils.scope(name):
            self.build_embedding(params)
            self.encoding = modules.PositionalEmbedding()
            self.encoder = HeadWiseTransformerEncoder(params)
            self.decoder = HeadWiseTransformerDecoder(params)
    ...
    @staticmethod
    def base_params_v2():
        params = HeadWiseTransformer.base_params()
    ...
    @staticmethod
    def big_params_v2():
        params = HeadWiseTransformer.base_params_v2()
        params.hidden_size = 1024
        params.filter_size = 4096
        params.num_heads = 16
        params.residual_dropout = 0.3
    ...
    def default_params(name=None):
        if name == "base":
            return HeadWiseTransformer.base_params()
        elif name == "base_v2":
            return HeadWiseTransformer.base_params_v2()
        elif name == "big":
            return HeadWiseTransformer.big_params()
        elif name == "big_v2":
            return HeadWiseTransformer.big_params_v2()
        elif name == "test":
            return HeadWiseTransformer.test_params()
        else:
            return HeadWiseTransformer.base_params()

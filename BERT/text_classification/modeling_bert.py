...
class WeightedBertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.expand_kappa_norm = config.expand_kappa_norm
        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
    ...
    def forward(
        ...
        if self.enable_kappa and self.enable_alpha:
            ...
            if self.expand_kappa_norm:
                normalized_kappa = normalized_kappa * self.num_attention_heads
...
class WeightedBertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.num_attention_heads = config.num_attention_heads
        self.enable_alpha = config.enable_alpha
        self.expand_alpha_norm = config.expand_alpha_norm
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        if self.enable_alpha:
            ...
            if self.expand_alpha_norm:
                normalized_alpha = normalized_alpha * self.num_attention_heads
...
class WeightedBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
...
class WeightedBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([WeightedBertLayer(config) for _ in range(config.num_hidden_layers)])
...
class WeightedBertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = BertEmbeddings(config)
        self.encoder = WeightedBertEncoder(config)
